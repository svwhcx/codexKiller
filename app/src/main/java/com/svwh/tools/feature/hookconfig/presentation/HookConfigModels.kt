...
internal fun defaultQuickConfigGroups(): List<HookQuickConfigGroup> {
    return listOf(
        ...
        HookQuickConfigGroup(
            ...
            items = listOf(
                HookQuickConfigItem("assets_read", "Assets 资源读取监听"),
                HookQuickConfigItem("file_read", "文件读取监听"),
                HookQuickConfigItem("file_write", "文件写入监听"),
                HookQuickConfigItem("file_delete", "文件删除监听"),
                HookQuickConfigItem("shell_exec", "shell 命令执行监听"),
            ),
        ),
        HookQuickConfigGroup(
            id = "interaction",
        ...
        HookQuickConfigGroup(
            ...
            items = listOf(
                HookQuickConfigItem("dialog_position", "弹窗定位"),
                HookQuickConfigItem(
                    id = "dialog_block_keywords",
                    title = "屏蔽关键词弹窗（点击设置内容）",
                    summaryValues = listOf("升级", "广告", "风险提示", "隐私授权"),
                ),
                HookQuickConfigItem("text_value", "控件文本赋值记录"),
                HookQuickConfigItem("activity_record", "Activity 记录"),
