...
function log(message) {
    console.log("[killer-frida] " + message);
}

function ___getRootPath() {
    return _SCRIPT_PATH_.replace(/\/scripts\/?$/, "");
}

function ___getScriptListPath() {
    return ___getRootPath() + "/sPath";
}

function ___scriptNameFromPath(path) {
    const fileName = path.slice(path.lastIndexOf("/") + 1);
    return fileName.endsWith(".js") ? fileName.slice(0, -3) : fileName;
}

function ___wrapScript(source) {
    return "(() => {\n" + source + "\n})()";
}

function ___readScriptList() {
    try {
        return File.readAllText(___getScriptListPath())
            .split("\n")
            .map((line) => line.trim())
            .filter((line) => line.length > 0);
    } catch (error) {
        console.error("cannot read script list: " + error);
        return [];
    }
}

function ___loadScript(path) {
    const name = ___scriptNameFromPath(path);
    try {
        const source = File.readAllText(path);
        Script.evaluate(name, ___wrapScript(source));
        log("loaded " + name);
    } catch (error) {
...
function ___main() {
    const scripts = ___readScriptList();
    if (scripts.length === 0) {
        return;
    }
    scripts.forEach(___loadScript);
}
