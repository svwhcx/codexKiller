...
## Hook 配置详情页
无环境、有环境 App 列表项支持点击进入对应 App 的 Hook 配置页面：
- 页面顶部展示返回、App 名称、包名、环境类型，以及保存、刷新、运行入口。
- 配置页包含 `快捷配置`、`用户配置`、`Frida`、`日志` 四个页签，支持左右滑动；点击页签使用 `scrollToPage` 直接切换，不播放跨页滑动动画。
- `快捷配置` 以 `HookQuickConfigGroup` 和 `HookQuickConfigItem` 抽象配置分组与配置项，支持大量配置项扩展。
- 快捷配置首版覆盖算法分析、网络环境、行为记录、交互监听、UI、存储与组件、反调试等分组。
- 配置项支持开关、说明文案、可点击详情提示、已设置关键词摘要，为后续 class、方法签名、变量修改、弹窗关键词拦截等真实 Hook 规则接入预留结构。
