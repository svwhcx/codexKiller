...
public class FileReadHookInvocation {

    public static void hook(HookLogWriter logWriter) throws NoSuchMethodException {
        Member member = FileInputStream.class.getDeclaredMethod("read", byte[].class, int.class, int.class);
        Pine.hook(member, new MethodHook() {
            @Override
            public void afterCall(Pine.CallFrame callFrame) throws Throwable {
                super.afterCall(callFrame);
                Field field = ReflectionHelper.findField(FileInputStream.class, "path");
                ...
                if (path != null) {
                    ...
                    Log log = new Log();
                    log.setTitle("读取文件");
                    log.setType(HookConfigTypeEnum.FILE_READ);
